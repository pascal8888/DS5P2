...
value <- c(1000000,1000,1000000000,1)
key <- c("M","K","B","0")
dmg_mplyr <- list()
dmg_mplyr <- setNames(as.list(value), key)
df_property_prop <- group_by(df_property, EVTYPE) %>% summarize(sum(PROPDMG))
df_property_crop <- group_by(df_property, EVTYPE) %>% summarize(sum(CROPDMG))
names(df_property_prop) <- c("EVTYPE","PROPDMG")
names(df_property_crop) <- c("EVTYPE","CROPDMG")
```
